...
class GetAllFriendRequestReceived{
    public static function resolve($args) : array{
        if(isset($args['auth']))
        {
            ...
            return DB::query("
                    SELECT `account_bound`.`id_sender` AS `id_account`, `account_bound`.`updated_at` AS `date`, `id_account_bound` AS `id_request`
                    FROM `account_bound`
                    INNER JOIN `login_session` ON `id_receiver` = `id_account`
