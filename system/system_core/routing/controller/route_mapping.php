...
class RouteMapping{
    ...
    public static function getAllMappedData(string $method, URLFormatting $formatted_url) : ?array{
        return self::$mapper->$method->uri_mapping[$formatted_url->path];
    }
    ...
    private static function mapController(string $method, string $url_path, string $config_path, string $module_path) : MiddlewareMapping{
        ...
        self::$mapper->$method->uri_mapping->push($url_path, new RoutingMappedData($config_path, $module_path, $routing_mapped_data_extension));
        return new MiddlewareMapping($method, $url_path, count(self::$mapper->$method->uri_mapping[$url_path]) - 1);
    }
